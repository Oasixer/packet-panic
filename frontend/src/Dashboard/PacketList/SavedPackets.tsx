...
export type SavedPacketsProps = {
  allPackets: Signal<DisplayPacket[]>;
};

export default class SavedPackets extends Component<SavedPacketsProps> {
  render() {
    // i wanna be able to filter by pretty much any element of the damn packet...
    const { allPackets } = this.props;
    const savedPackets: Signal<DisplayPacket[]> = computed(() => {
      return allPackets.value.filter((x) => {
        return x.saved;
      });
    });
